...
import com.notfound.timetrackserver.mapper.AdminMapMapper;
...
public class AdminMapServiceImpl implements AdminMapService {
    ...
    private void enrichPoiDetails(List<AdminMapPoiVO> pois, String commentStatus) {
        ...
        Map<Long, List<com.notfound.timetrackpojo.vo.AdminMapFavoriteVO>> favoritesByPoi =
                adminMapMapper.listPoiFavorites(poiIds).stream()
                        .filter(item -> item.getRelatedPoiId() != null)
                        .collect(Collectors.groupingBy(com.notfound.timetrackpojo.vo.AdminMapFavoriteVO::getRelatedPoiId));
        Map<Long, List<com.notfound.timetrackpojo.vo.AdminMapCommentVO>> commentsByPoi =
                adminMapMapper.listPoiComments(poiIds, commentStatus).stream()
                        .filter(item -> item.getRelatedPoiId() != null)
                        .collect(Collectors.groupingBy(com.notfound.timetrackpojo.vo.AdminMapCommentVO::getRelatedPoiId));
        Map<Long, List<com.notfound.timetrackpojo.vo.AdminMapMediaVO>> mediaByPoi =
                adminMapMapper.listPoiMedia(poiIds).stream()
                        ...
                        .collect(Collectors.groupingBy(com.notfound.timetrackpojo.vo.AdminMapMediaVO::getPoiId));
