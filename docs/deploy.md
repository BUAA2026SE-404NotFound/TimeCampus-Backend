...
部署方式为服务器本地 `git pull origin main` 后构建后端 Jar。管理端前端已迁出后端仓库；如需同步发布前端，将外部前端仓库构建出的 `dist` 路径通过 `UI_DIST_DIR` 传给部署脚本。Nginx 对外提供管理端静态文件并反向代理 `/api/v1/**` 到 Spring Boot。
...
```bash
cd ~
git clone git@github.com:<owner>/<repo>.git TimeCampus-Backend
cd ~/TimeCampus-Backend
mkdir -p ~/TimeCampus-Backend/app/config
mkdir -p ~/TimeCampus-Backend/app/ui
...
```bash
cp ~/TimeCampus-Backend/timecampus-server/src/main/resources/application-prod-example.yaml \
   ~/TimeCampus-Backend/app/config/application-prod.yaml
nano ~/TimeCampus-Backend/app/config/application-prod.yaml
```
...
- Spring Boot 会自动读取当前目录下的 `config/application-prod.yaml`，也就是 `~/TimeCampus-Backend/app/config/application-prod.yaml`。
...
```bash
mysql -h 127.0.0.1 -u your_mysql_user -p timecampus < \
  ~/TimeCampus-Backend/timecampus-server/src/main/resources/sql/schema.sql
```
...
```bash
mysql -h 127.0.0.1 -u your_mysql_user -p timecampus < \
  ~/TimeCampus-Backend/timecampus-server/src/main/resources/sql/dev-seed-buaa.sql
```
...
- `/`：服务 `~/TimeCampus-Backend/app/ui` 下的 Vue 管理端。
...
GitHub Actions CD 执行的核心逻辑如下，必要时可在服务器手动运行。若要同时更新管理端静态文件，先在外部前端仓库构建 `dist`，再设置 `UI_DIST_DIR=/path/to/timecampus-ui/dist`：
...
mkdir -p ~/TimeCampus-Backend/app/ui
cp timecampus-server/target/*.jar ~/TimeCampus-Backend/app/app.jar

if [ -n "${UI_DIST_DIR:-}" ]; then
  rm -rf ~/TimeCampus-Backend/app/ui/*
  cp -R "$UI_DIST_DIR"/. ~/TimeCampus-Backend/app/ui/
fi

cd ~/TimeCampus-Backend/app
...
```bash
tail -n 200 ~/TimeCampus-Backend/app/app.log
```
